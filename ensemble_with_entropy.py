...
def JeffreyDiv(logit, logit2, logit_stat, logit2_stat):
    #logit = image, logit2 = cap
    kl_loss = torch.nn.KLDivLoss(reduction="batchmean")

    input = F.log_softmax(logit, dim=-1)
    target = F.softmax(logit2, dim=-1)
    beta = logit_coefficient(logit)
    
    input_ = F.log_softmax(logit2, dim=-1)
    target_ = F.softmax(logit, dim=-1)
    alpha = logit_coefficient(logit2)
    
        
    logit_stat = torch.cat([logit_stat, torch.tensor([beta],dtype=torch.float32).cuda()])
    logit2_stat = torch.cat([logit2_stat, torch.tensor([alpha], dtype=torch.float32).cuda()])
    #normalize coefficient in each modality
    img_normalized = normalize(logit_stat, p=1.0, dim = 0)[-1]
    cap_normalized = normalize(logit2_stat, p=1.0, dim = 0)[-1]
     
    one = cap_normalized * kl_loss(input, target.detach()) #image를 caption에 맞춤
    two = img_normalized * kl_loss(input_, target_.detach())#caption을 Image에 맞춤
    return one + two
    ...
    for j in range(args.tta_steps):
        ...
        with torch.cuda.amp.autocast():
            ...
            if retrieved_Caption == None:
                # print("retrieve caption none ", index, retrieved_Caption)
                return None, None 
            ...
            Entloss = conf_cap+conf_img

            #loss, penalty
            # print(JeffreyDiv(output_img, output_caption))
            # JeffDiv = JeffreyDiv(output_img, output_caption, mean_stat, cap_mean_stat)
            # print(Entloss, JeffDiv )
            # print(JeffDiv)

            # print(conf_img, conf_cap)
        optimizer.zero_grad() 
        scaler.scale(Entloss).backward()
...
def test_time_adapt_eval(val_loader, model, cap_cache, optimizer, optim_state, scaler, save_result=False, set_id='', args=None):
    ...
    for i, (c_idx, images, target, imagepath) in tqdm(enumerate(val_loader)): 
        ...
        mean_stat_clone = mean_stat.clone().detach()
        cap_mean_stat_clone = cap_mean_stat.clone().detach()
        retrieved_Caption, caption_logit = test_time_tuning(model, cap_cache, image, optimizer, scaler, imagepath[0], mean_stat_clone, cap_mean_stat_clone, args, index=c_idx.item(), save_result = save_result)
        if retrieved_Caption == None:
            cnt_empty +=1 
            # with torch.no_grad():
            #     with torch.cuda.amp.autocast():
            ...
            # print(img_normalized, cap_normalized)
            ...
            # print(img_logit_norm, cap_logit_norm)
            correct_ = accuracy(img_normalized * img_logit_norm + cap_normalized * cap_logit_norm , target, topk=(1, 2, 3, 4, 5), caption=None, logger=None).item()
...
def main_worker(gpu, args):
    ...
    for set_id in datasets:
        ...
        for retrieve_K in [32]:
            ...
            results = test_time_adapt_eval(val_loader, model, cap_cache, optimizer, optim_state, scaler, Dict, set_id, args)
            if save_cache :
                path = args.cap_cache
                os.makedirs(path, exist_ok=True)
