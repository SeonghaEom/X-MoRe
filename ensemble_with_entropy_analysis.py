...
def test_time_adapt_eval(val_loader, model, cap_cache, optimizer=None, optim_state=None, scaler=None, save_result=False, set_id='', args=None, label_features = None):
    ...
    for i, (c_idx, images, target, imagepath) in tqdm(enumerate(val_loader)): 
        ...
        if retrieved_Caption == None:
            ...
            ## evaluate image and save result
            save_result['image_correct'].append(int(accuracy(output_img, target).item()))
            
            ## evaluate caption and save result
            save_result['caption_correct'].append(int(accuracy(caption_logit, target).item()))
            
            ## evaluate ensemble and save result
            alpha, ent = logit_coefficient(output_img)
            save_result['ent_img'].append(ent.item())
            beta, ent = logit_coefficient(caption_logit)
            save_result['ent_cap'].append(ent.item())
            ...
            correct_ = accuracy(alpha * img_logit_norm + beta * cap_logit_norm , target).item()
            
            
            save_result['c_idx'].append(c_idx.item())
            save_result['imagepath'].append(imagepath[0])
            save_result['ensemble_correct'].append(correct_)
            save_result['hat_alpha_ratio'].append(img_normalized.item()/cap_normalized.item())
            # save_result['hat_alpha_cap'].append(cap_normalized.item())

            # if (i+1) % 1000 == 0 or (i+1) == len(val_loader):
...
def main_worker(gpu, args):
    ...
    for set_id in datasets:
        ...
        # retrieve_K = 8
        path = './notebook/analysis/{}/zsh/{}/{}'.format(args.arch, args.seed, args.retrieve_K)
        if args.load: path = './notebook/analysis/pretrained/{}/{}/{}'.format(args.arch, args.seed, args.retrieve_K)
        ...
        path = './notebook/analysis/conf_adj/{}/zsh/{}/{}'.format(args.arch, args.seed, args.retrieve_K)
        if args.load: path = './notebook/analysis/pretrained/{}/{}/{}'.format(args.arch, args.seed, args.retrieve_K)
